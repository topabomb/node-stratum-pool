...
//_this.emit('hualei_msg',message);
	...
	//_this.emit('hualei_msg',message);
		...
		//todo:来自job的回调
                if (!considerBan(result)){
                    if(!result){
                         _this.emit('hualei_msg','send response error :error:'+JSON.stringify(error));
		                _this.emit('hualei_msg','send response error :result:'+JSON.stringify(result));
                    }
                    if(error&&error[0]==21){
                        _this.emit('hualei_msg','send response job error :result:'+JSON.stringify(result));
                        sendJson({
                            id: message.id,
                            result: true,
                            error: null
	...
	//_this.emit('hualei_msg','sendJson:'+JSON.stringify(response));
    }
