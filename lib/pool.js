...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function SetupJobManager(){
		...
		emitLog('pools onshare:'+ JSON.stringify(shareData) );
		...
		//emitLog('CheckBlockAccepted(shareData.blockHash, function(isAccepted, tx):'+isAccepted+'TX:'+tx);
    ...
    function StartStratumServer(finishedCallback){
		...
		//emitLog('on submit :' + params.jobId);
		...
		}).on('hualei_msg', function(err) {
                //emitWarningLog('hualei_msg: ' + client.getLabel() + ': ' + JSON.stringify(err));
		});
        });
    }
    ...
    function SetupBlockPolling(){
        ...
        blockPollingIntervalId = setInterval(function () {
            GetBlockTemplate(function(error, result, foundNewBlock){
                if (foundNewBlock)
                    emitLog('Block notification via RPC polling!hual');
            });
        }, pollingInterval);
    }
